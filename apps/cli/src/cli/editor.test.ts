...
Deno.test("resolveRecordEditDirectory writes inside <repoRoot>/.changelog", () => {
  // Given any repo root
  ...
  // Then the path is rooted at the repo and sits under .changelog (so
  // VSCode treats it as part of the trusted workspace, matching git's
  // pattern of writing to .git/COMMIT_EDITMSG)
  assertEquals(editDirectory, join("/some/repo", ".changelog"));
});
...
Deno.test("resolveRecordEditDirectory threads the supplied repo root verbatim", () => {
  ...
  // Then the result preserves the supplied root and joins .changelog
  // with the platform separator (assertEquals via join below normalizes)
  assertEquals(
    editDirectory,
    join("/Users/example/Path With Spaces/my-repo", ".changelog"),
  );
});
