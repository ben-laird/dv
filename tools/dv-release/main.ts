// dv's *own* release plugin — repo-local, not an example.
//
// Started as a copy of examples/plugins/deno/main.ts (which is
// designed to be copied per examples/CLAUDE.md), then specialised:
// - The `release` op actually runs `deno publish` instead of
//   stubbing the success.
// - The `name` field in the `info` response identifies this
//   plugin as `dv-release` rather than the example's `deno`.
//
// Every other op is a verbatim copy of the example. We don't
// import + re-dispatch because (a) the example is a single-file
// dispatcher with no exports, (b) the contract is stable enough
// that drift is unlikely, and (c) examples/CLAUDE.md is explicit
// that example plugins are starting points, not maintained
// dependencies. Copy-and-specialise is the documented pattern.
//
// Wired in via .dv/config.yaml:
//   discovery:
//     plugins:
//       - match: ["apps/*", "packages/*"]
//         use:
...
// Runs `deno publish` from the package directory. dv set DV_REPO_ROOT
// + DV_PACKAGE_PATH + DV_PACKAGE_NAME + DV_NEW_VERSION + DV_GIT_TAG
// for us; the package directory holds the deno.json the publish
// reads. We don't pass --token explicitly — `deno publish` picks up
// OIDC credentials from GitHub Actions automatically when running on
// a configured JSR-trusted workflow.
//
// Per specs/plugin-contract.md, this op is the only one where
// `{ ok: false }` is a valid (non-error) response shape: publish
// failures are *data* — dv records the per-package outcome, the tag
// stays minted, the run continues, and `dv release --force` can
// retry. Throwing instead of returning ok:false would be a contract
// violation.
...
async function runRelease(): Promise<void> {
  ...
  if (!packagePath) {
    console.log(
      JSON.stringify({
        ok: false,
        message: "DV_PACKAGE_PATH is required",
      }),
    );
    Deno.exit(1);
  }

  // `deno publish` checks the deno.json's `name` + `version` against
  // the JSR registry; --allow-dirty is needed because dv has already
  // staged (but not necessarily committed) the version commit and
  // release tag by the time this op runs. Quiet so JSR's progress
  // chatter doesn't leak into stdout (which dv parses as JSON).
  const publishResult = await new Deno.Command("deno", {
    args: ["publish", "--allow-dirty", "--quiet"],
    cwd: packagePath,
    stdout: "piped",
    stderr: "piped",
  }).output();
  ...
  if (!publishResult.success) {
    const stderrText = new TextDecoder().decode(publishResult.stderr).trim();
    console.log(
      JSON.stringify({
        ok: false,
        message: `deno publish failed for ${packageName}@${newVersion} (exit ${publishResult.code}): ${stderrText || "<no stderr>"}`,
      }),
    );
    return;
  }
  ...
  console.log(
    JSON.stringify({
      ok: true,
      published: true,
      message: `published ${packageName}@${newVersion} to JSR`,
    }),
  );
}
